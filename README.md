...
3. Results are persisted in **MongoDB** (Atlas in prod) and shown in a sortable table at `/results`. Re-analyzing the same URL is a no-op — the existing record is returned.
...
| ORM | **Mongoid 9** (no ActiveRecord) | The brief explicitly required MongoDB; Mongoid is the idiomatic Rails ↔ Mongo mapper, and Atlas is free at the M0 tier. |
...
- Ruby **3.3+** (project is built on 3.4.1 via `mise`)
- Node **20 LTS** or **22 LTS** (project uses 22)
- A running MongoDB — either local (`brew services start mongodb-community`) or an Atlas cluster
...
cd api
bundle install
cp .env.example .env       # then fill in MONGODB_URI, GNEWS_API_KEY, OPENAI_API_KEY
...
cd web
npm install
cp .env.example .env       # default VITE_API_URL=http://localhost:3000 is fine
npm run dev
```

The Vite dev server listens on `http://localhost:5173` and proxies `/api/*` to the backend, so CORS isn't even needed for local dev.
...
- **Repo root**: `api/`
- **Build**: `bundle install && bundle exec rake db:mongoid:create_indexes`
- **Start**: `bundle exec rails server -p $PORT -e production`
- **Health check path**: `/up`
- **Env vars**: `MONGODB_URI`, `GNEWS_API_KEY`, `OPENAI_API_KEY`, `FRONTEND_ORIGIN`, `RAILS_MASTER_KEY`, `RAILS_ENV=production`, `RAILS_LOG_TO_STDOUT=true`
...
- **Publish dir**: `dist`
- **Env var**: `VITE_API_URL=<the backend URL from above>`

After both are live, set `FRONTEND_ORIGIN` on the backend to the frontend's URL and trigger a redeploy so CORS allows the prod origin.
